...
(function ($) {
    ...
    $.fn.wysiwyg = function (userOptions) {
        ...
        $(window).bind('touchend', function (e) {
            var isInside = (editor.is(e.target) || editor.has(e.target).length > 0), currentRange = editorWysiwyg.getCurrentRange(), clear = currentRange && (currentRange.startContainer === currentRange.endContainer && currentRange.startOffset === currentRange.endOffset);
            if (!clear || isInside) {
                editorWysiwyg.saveSelection();
                editorWysiwyg.updateToolbar();
            }

...
namespace MiniWeb.Core
{
    ...
    public static class MiniWebExtensions
    {
        ...
        public static IEndpointRouteBuilder MapMiniwebApiRoute(this IEndpointRouteBuilder endpoints)
        {
            var config = endpoints.ServiceProvider.GetRequiredService<IOptions<MiniWebConfiguration>>().Value;
            endpoints.MapControllerRoute("miniwebapi", $"{config.ApiEndpoint.Substring(1)}{{action}}", new { controller = "MiniWebApi" });
            return endpoints;
        }

        public static IEndpointRouteBuilder MapMiniwebRazorWithFallback(this IEndpointRouteBuilder endpoints)
        {
            var config = endpoints.ServiceProvider.GetRequiredService<IOptions<MiniWebConfiguration>>().Value;
            endpoints.MapMiniwebApiRoute();

            //this fallback page handles all the Miniweb Pages
            endpoints.MapFallbackToPage("{*pageUrl}", config.RazorFallbackPageRoute);
            return endpoints;
        }

        public static IEndpointRouteBuilder MapMiniWebSite(this IEndpointRouteBuilder endpoints)
        {
            var config = endpoints.ServiceProvider.GetRequiredService<IOptions<MiniWebConfiguration>>().Value;
            endpoints.MapControllerRoute("miniwebapi", $"{config.ApiEndpoint.Substring(1)}{{action}}", new { controller = "MiniWebApi" });
            endpoints.MapControllerRoute("miniwebsociallogin", config.Authentication.SocialLoginPath.Substring(1), new { controller = "MiniWebPage", action = "SocialLogin" });
            endpoints.MapControllerRoute("miniweblogin", config.Authentication.LoginPath.Substring(1), new { controller = "MiniWebPage", action = "Login" });
            endpoints.MapControllerRoute("miniweblogout", config.Authentication.LogoutPath.Substring(1), new { controller = "MiniWebPage", action = "Logout" });
            endpoints.MapControllerRoute("miniweb", "{*url}", new { controller = "MiniWebPage", action = "Index" }, constraints: new { url = new MiniWebRouteConstraint(config.PageExtension, config.PageExtensionForce) });
